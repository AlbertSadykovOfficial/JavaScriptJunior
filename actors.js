...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    move() {
        let moveCounter = 0;    // чтоб узнать на сколько клеток поля произошло смещение ***
        let targetDone = false ;

        while(targetDone == false){     // Изменяю координаты, пока на что-либо не наткнусь
            if ( PLAN[this.xy.y + this.dir.y][this.xy.x + this.dir.x] == Cell.EMPTY ) {
                this.xy.x += this.dir.x;
                this.xy.y += this.dir.y;
            } else {  
                    //  Классифицирую объект после столкновения столкнуся
                if ( PLAN[this.xy.y + this.dir.y][this.xy.x + this.dir.x] == Cell.WALL )  
                    return ActionType.GONE ;
                else  
                    return ActionType.APPLY ;    

            targetDone = true;         
            }
            moveCounter++;
        }
       fireballMove(this.id,this.xy,this.dir,moveCounter); // ***
    }
